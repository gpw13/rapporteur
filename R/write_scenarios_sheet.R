...
write_scenario_sheet <- function(df,
  ...
  unique_scenarios <- sort(unique(df[[scenario]]))
  ...
  df_iso_scenarios <- df %>%
    dplyr::select(dplyr::any_of(c(scenario, ind, year, value, type_col))) %>%
  ...
  df_iso_scenarios_wide <- df_iso_scenarios %>%
    dplyr::group_by(.data[[scenario]], .data[[ind]]) %>%
    dplyr::select(-dplyr::any_of(c(type_col))) %>%
    tidyr::pivot_wider(names_from = year, values_from = .data[[value]]) %>%
    dplyr::ungroup()

  df_iso_scenarios_nice_ind_wide <- nice_inds %>%
    dplyr::left_join(df_iso_scenarios_wide, by = c("ind" = ind, "scenario" = scenario)) %>%
    dplyr::arrange(order, factor(.data[[scenario]], scenarios_order)) %>%
    dplyr::ungroup() %>%
    dplyr::select(scenario, "short_name", years_range_chr)
